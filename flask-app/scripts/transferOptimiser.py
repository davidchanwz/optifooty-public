...
data_file_path = os.path.join(
    script_dir, "..", "data", "user_transfer_info.json")
...
def get_transfer_details(initial_lineup, optimal_squad):
    ...
    transferred_out = [
        player for player in initial_lineup if player not in optimal_squad]

    # Find transferred in players
    transferred_in = [
        player for player in optimal_squad if player not in initial_set]

    # Create a mapping from player names to their predicted points
    name_to_points = {row['player_name']: row['predicted_points']
